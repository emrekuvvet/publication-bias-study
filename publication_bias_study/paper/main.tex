...
\textit{Second}, as supporting context, the paper provides cross-sectional evidence on the \textit{absence of detectable} direction-based sorting between top-journal articles and NBER working papers.  This null result is identified from two largely non-overlapping populations and cannot be interpreted as evidence of editorial neutrality; it is consistent with journals selecting on methodological quality rather than finding direction.  The AFA conference baseline corroborates the null: directional papers carry no significant association with eventual publication ($\hat\beta = 0.327$, $p = 0.24$)---though supply-side submission decisions cannot be ruled out.
