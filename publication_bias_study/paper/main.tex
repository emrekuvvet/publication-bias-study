...
allowing a direct comparison of test-statistic distributions rather than merely
...
The strongest predictor of top-journal publication is quasi-experimental
...
cross-sectional evidence on the \textit{absence of detectable} direction-based
sorting between top-journal articles and NBER working papers in the
government-intervention finance literature---though we stress that this
null result is identified from two non-overlapping populations and cannot
be interpreted as evidence of editorial neutrality per se.  The result is
consistent with finance journals selecting primarily on methodological rigor
rather than on finding direction, conditional on the NBER being a reasonable
comparison corpus.

The paper also contributes methodologically to the literature on identifying
publication bias.  Unlike approaches that infer bias solely from the internal shape
of published p-value distributions, our NBER working paper baseline allows
a direct comparison of test-statistic distributions across two corpora at
different stages of the publication pipeline, rather than merely inferring the
pre-publication shape from published p-values.  The AFA conference comparison provides a consistency check:
...
$p = 0.24$), consistent with the absence of
a publication-stage direction premium.  Both pre-publication baselines (NBER and AFA) support the same
conclusion: no evidence of cross-sectional significance-sorting---though supply-side
...
Table~\ref{tab:quasiexp_direction} disaggregates publication rates by the joint
distribution of direction and identification strategy.  Within every direction
category, QE papers publish at materially higher rates than non-QE papers
(positive: 55.6\% vs.\ 34.7\%; negative: 52.2\% vs.\ 39.0\%; null: 75.0\%
vs.\ 36.4\%).  The direction-specific publication rates are nearly identical
...
\begin{table}[ht]
\centering
\caption{Publication Rates by Finding Direction and Identification Strategy}
...
\smallskip
\footnotesize
\textit{Notes.} Each cell shows the publication rate (top-3 finance journal) with
...
identically to the theoretical papers.  The \textit{NBER sample} therefore represents the pre-publication distribution
...
exhibit but published papers exhibit more strongly.  A plausible
interpretation is that standalone t-statistics are more amenable to
selective presentation during revision (e.g., retaining only specifications
whose key t-statistic clears 1.96 in a summary column) than are
coefficient-SE pairs that appear as the primary tabulation.  This is an
...
difference in publication rates at 80\% power is approximately 18 percentage
...
threshold.  This interpretation is \textit{consistent with} equal publication rates
...
within each QuasiExp tier the publication rate is flat across direction
...
We examine whether top finance journals exhibit direction-based sorting in the
government-intervention literature, using a cross-sectional comparison of two
largely non-overlapping corpora: 291 in-scope papers spanning 2000--2024
(134 published, 157 NBER working papers).  We find no \textit{detectable}
direction-based sorting between the two corpora.  Corpus-membership rates are
nearly identical across positive-finding (45.6\%), negative-finding (46.1\%),
and null-finding (46.7\%) papers ($\chi^2 = 0.017$, $p = 0.992$).  A binary
...
specifications.  We emphasize that these corpus-membership rates depend on
sample construction and are not estimates of actual acceptance probabilities;
the comparison reflects differences between two distinct populations rather
than the same papers tracked through the editorial filter.

The strongest predictor of corpus membership in our sample is the use
...
this coding has not been independently validated at the granular level).
This identification-quality association is consistent with editorial reward for
methodological rigor rather than for finding direction.

A complementary analysis of within-paper test statistic distributions
(\citealt{Brodeur2016} caliper test; 7,761 $|z|$-statistics from 121 papers, 108 as permutation clusters)
...
NBER working papers shows no bunching (full-sample ratio $= 1.01$,
$p = 0.849$; negative-finding ratio $= 1.00$, $p = 0.964$), while the overall
significance rate is nearly identical across the two corpora (57.1\% vs.\
55.5\%).  This comparison localizes the distributional difference to the
published corpus, though compositional differences between the two corpora
cannot be ruled out as an alternative explanation.  An important qualification
is that when the caliper analysis is restricted to rows with verifiable
coefficient-SE pairs (5,510 published; 6,788 NBER), neither corpus shows
significant bunching (published ratio $= 0.81$, NBER $= 0.95$); the overall
bunching is concentrated in z-stat-only rows that cannot be independently
verified (Appendix~\ref{app:coefse_caliper}).  This limits the strength of
the caliper interpretation.  We are not
aware of a prior published-vs.-working-paper caliper comparison in finance,
though the approach extends naturally from the broader caliper literature
\citep{Brodeur2016,Brodeur2023}.

The joint pattern shows the \textit{absence} of detectable directional sorting
between the two corpora, alongside \textit{within-paper} threshold bunching
concentrated in the published record---a pattern consistent with
specification-search pressure during revision.  We stress that compositional
differences between NBER affiliates and the broader population of published
authors could partly explain both results, and that neither claim about
mechanism is directly identified.
