...
contain between $1.5$ and $2.0$ times as many test statistics that land just
above the conventional significance threshold as those that land just below
it---a sign that researchers and editors favor results that clear the
significance bar.  \citet{Franco2014} tracked NSF-funded social science
experiments from registration through to publication and found that 65\% of
null results were never published, compared with only 21\% of significant
results---a roughly three-to-one publication advantage for significant
findings.  \citet{DellaVigna2022} report similar disparities in a sample of
...
clustering of test statistics just below the 5\% threshold is also more
moderate in our published sample (a ratio of 1.20) than in the top economics
journals ($1.5$--$2.0\times$), and the same measure applied to NBER working
...
Two caveats qualify these conclusions.  First, NBER working papers and
published articles are largely non-overlapping groups, so our comparison
captures differences in the composition of research circulating before
submission rather than the editorial filter applied to the same papers.
Second, NBER researchers are drawn from a select group at leading
universities; if their papers are inherently more publishable, our design
would understate any true direction premium in the broader research
population.  Both limitations bias toward the null, making our findings
a conservative lower bound rather than a precise point estimate of
