...
%% ---- Title block --------------------------------------------
\title{Threshold Bunching in Published Finance Research:\\
       Pre-Publication Evidence from Government-Intervention Studies%
...
\begin{singlespace}
\noindent\small
We document a sharp contrast between published and pre-publication test-statistic
distributions in the government-intervention literature.  Applying the
\citet{Brodeur2016} caliper test to 7,761 test statistics from 121 articles in
the \JF{}, \RFS{}, and \JFE{} (108 papers with in-caliper-window statistics, which
serve as the permutation and bootstrap clusters) reveals significant threshold bunching
just below $|z|=1.96$ (caliper ratio $1.20$; naive $p = 0.002$; paper-level
permutation $p = 0.001$), whereas the identical pipeline applied to 8,351 test
statistics from 125 NBER working papers (106 as clusters) on the same topic
finds no bunching at all (ratio $1.01$, $p = 0.849$).  The contrast is sharpest for negative-finding papers
(published ratio $1.52$, permutation $p < 0.001$; cluster-bootstrap 95\% CI
$[0.81,\,2.67]$; NBER ratio $1.00$, $p = 0.964$) and survives restriction to
non-control variable coefficients (non-control ratio $1.625$).  Because
overall significance rates are nearly identical across samples (57.1\%
vs.\ 55.5\%), the bunching is not explained by differences in statistical power;
it is concentrated in the published record and absent from the NBER working
papers, consistent with within-paper specification pressure at the revision
stage---though compositional differences between the two corpora cannot be
ruled out.  An important qualification: when restricted to rows with
verifiable coefficient-SE pairs (5,510 published rows), the caliper ratio
reverses to 0.81 and shows no significant gap versus NBER (0.95); the
overall bunching is concentrated in z-stat-only rows.
Note that the NBER and journal samples are largely
non-overlapping cross-sections (only $\approx$12\% of published papers are
matched to NBER counterparts), so this comparison reflects differences between
two distinct corpora rather than the same papers tracked pre- and
post-publication.

As supporting context, a cross-sectional comparison of corpus-membership
probabilities across 134 top-journal articles and 157 NBER working papers
(2000--2024) finds no detectable directional sorting between corpora:
positive-finding papers appear in top journals at 45.6\%, negative-finding at
46.1\%, and null-finding at 46.7\% ($p = 0.992$); a binary directional probit
coefficient is $-0.020$ (s.e.\ 0.182, $p = 0.91$), robust across six
specifications.  These rates are corpus-membership ratios determined by our
sample construction, not estimates of true journal acceptance
probabilities.  The strongest predictor of corpus membership is a
quasi-experimental identification strategy (coefficient $0.467^{***}$,
s.e.\ 0.157, $\approx$18 pp; this coding has not been independently validated
at the granular level).  The joint pattern shows no detectable directional
sorting between the two corpora, while the caliper comparison reveals
within-paper threshold bunching concentrated in the published record.
\end{singlespace}
\vspace{4pt}
...
test to 7,761 $|z|$-statistics from 121 published papers (of which 108 contribute
at least one in-caliper-window statistic and serve as permutation/bootstrap clusters),
we find significant full-sample threshold bunching (caliper ratio $= 1.20$; naive
$p = 0.002$; paper-level permutation $p = 0.001$), concentrated in \JFE{} (caliper
ratio $= 1.49$, $p < 0.001$) and strikingly asymmetric by direction:
\textit{negative-finding papers} show a caliper ratio of 1.52 (permutation
$p < 0.001$; cluster-bootstrap $p = 0.135$) while positive-finding papers show
no bunching (ratio $= 0.94$, $p = 0.565$).
...
statistics from 125 papers (106 as clusters).  The NBER caliper ratio is 1.01 ($p = 0.849$)
...
samples (published: $57.1\%$; NBER: $55.5\%$), a fact that is \textit{consistent
with} similar statistical power but does not fully establish it (equal significance
rates can coexist with different effect sizes, sample sizes, or model specifications).
The bunching does not appear to be a property of the underlying research designs
in this domain; it is concentrated in the published record and largely absent
from the NBER working papers, consistent with specification-search pressure
concentrated at the revision stage---though compositional differences between
the two corpora cannot be ruled out.
...
\textit{First and primarily}, it documents within-paper threshold bunching in published government-intervention finance research and shows this bunching is absent from NBER working papers on the same topics.  Using the \citet{Brodeur2016} caliper test and NBER full texts as a proxy for the pre-publication distribution, we provide a direct two-corpus comparison of test-statistic distributions---a design that avoids the purely inferential step required when the pre-publication distribution is unobserved.  The bunching is concentrated in negative-finding papers (caliper ratio $1.52$, $p < 0.001$) and in z-stat-only rows; it is absent from NBER counterparts (ratio $1.01$, $p = 0.849$), pointing to revision-stage specification search as a plausible mechanism.  An important qualification is that bunching is not detected in coefficient-SE rows (published ratio $0.81$), so the result should be treated as suggestive rather than definitive.

\textit{Second}, as supporting context, the paper provides cross-sectional evidence consistent with the absence of detectable direction-based sorting between top-journal articles and NBER working papers.  This result is identified from two largely non-overlapping populations and cannot be interpreted as evidence of editorial neutrality.  Critically, the design is underpowered: at 80\% power the minimum detectable effect is $\approx$22 percentage points in corpus-membership probability; effects smaller than this threshold are consistent with the data and cannot be ruled out.  The design rules out \textit{large}, implausibly strong directional sorting, not moderate asymmetries.  The AFA baseline carries a positive but insignificant point estimate ($\hat\beta = 0.327$, $p = 0.24$), diverging in sign from the NBER estimate ($-0.020$); both are within sampling noise given the small samples.
...
to test robustness to model choice; the same prompt was used in both runs so
that disagreements isolate model-choice sensitivity rather than prompt-choice sensitivity.
LLM coding in general is sensitive to exact prompt phrasing; we report the
per-category disagreement pattern below so readers can assess directional risk.
...
$\kappa$ statistics are reported in Table~\ref{tab:human_validation} in
...
We begin with the paper's primary empirical test: whether published articles in the government-intervention literature show systematic excess mass just below conventional significance thresholds, and whether that bunching is absent from NBER working papers covering the same research domain.  We apply the \citet{Brodeur2016} caliper test to the full distribution of test statistics extracted from published and NBER paper PDFs.
...
Fourth, we stratify the caliper by row type to assess whether the
overall result is driven by the unverifiable z-stat-only rows
(Appendix~\ref{app:coefse_caliper}).  Among the 5,510 published rows
with both a coefficient and a standard error, the caliper ratio is 0.81
(below $=$ 320, above $=$ 394; naive $p = 0.006$ against 1.0 from
below); the corresponding NBER ratio is 0.95 (below $=$ 416, above $=$
438; $p = 0.45$).  Neither corpus shows excess bunching in this clean
subset, and the two ratios are close.  Among the 2,251 published z-stat-only
rows, the caliper ratio is 2.55 ($p < 0.001$); the comparable NBER ratio
is 1.49 ($p = 0.019$).  The full-sample results therefore reflect
disproportionate bunching in standalone t-statistics, which both corpora
exhibit but published papers exhibit more strongly.  This is an
important qualification: the cleanest extraction rows show no
significant bunching gap, and caution is warranted in interpreting the
magnitude of the overall caliper contrast.
...
\medskip
\noindent\textbf{Mechanism: selective reporting vs. full-table extraction.}
...
We offer the following explanation, which is empirically supported by our extraction data.
Standard regression tables (the source of coef+SE rows) report \textit{all}
coefficient estimates from a given specification systematically --- including
insignificant ones.
Standalone t-statistics, by contrast, appear disproportionately in supplementary
analyses, robustness summary rows, and ``Panel B''-type tables where authors
have already \textit{selected} which test to present.
Consistent with this, 96.1\% of published z-stat-only rows in our extraction
come from non-primary-results tables (tables not classified as containing the
paper's main regression specification), compared to 3.9\% from main results tables.
This selective-reporting structure is itself a form of within-paper specification
search: the author chose to highlight the specific test statistic because it is
noteworthy or marginally significant.
Under this interpretation, bunching in z-stat-only rows captures the joint effect
of (a) selective \textit{reporting} of individual test statistics within a paper,
and (b) possibly, specification-search pressure at the revision stage.
The absence of bunching in coef+SE rows is consistent with primary regression
tables being systematically rather than selectively reported.
This mechanism also explains why both corpora show a z-stat-only ratio above one
(published: 2.55; NBER: 1.49) --- selective reporting of standalone test statistics
occurs in working papers too, though less intensely than in published papers.
...
negative conclusions---the bunching is \textit{consistent with} revision-stage
pressure: one interpretation is that anti-intervention papers face elevated
evidentiary demands---either self-imposed by authors anticipating scrutiny or
enforced by reviewers---leading to specification search until marginal results
clear the 5\% threshold.  A compositional alternative is also possible:
negative-finding published papers may differ from their NBER counterparts in
unobserved dimensions (empirical detail, research design complexity, vintage)
correlated with within-paper specification counts, and our cross-sectional
design cannot rule this out.  We do not claim to have established a causal mechanism; the data are consistent
with revision-stage specification pressure but alternative compositional
explanations remain open.  The cross-sectional comparison shows that the
directional pattern in caliper ratios is specific to the published corpus.
...
Taken together, the three inference procedures paint a consistent picture: the
caliper result for negative-finding papers is robust to the permutation test,
directionally consistent with the bootstrap and cross-group permutation, and
wholly absent from the NBER pre-publication baseline.
...
journal acceptance probabilities.)  Positive-finding papers appear in the
...
reference group for Columns~(2)--(6)---has only 25\% inter-rater agreement
...
as classified by our LLM pipeline applied to abstracts).
...
coefficient is $0.467^{***}$ (s.e.\ 0.157, $p = 0.003$), making it the dominant
predictor in this specification.  We note that the LLM-coded quasi-experimental
variable is likely correlated with author quality, topic type, and other dimensions
of paper quality, so the coefficient reflects a bundle of correlated attributes
rather than identification strategy in isolation.  Direction
...
sign.  The sign is unexpected if significance-sorting were operating: it implies
quasi-experimental negative-finding papers are \textit{less} likely to be
published than null papers.  We flag this as likely small-sample noise ($N=139$)
rather than evidence of an anti-intervention penalty; the subsample is too small
to reliably estimate differential direction effects, and the point estimate is
sensitive to which papers are classified as quasi-experimental.  Neither estimate
supports a positive direction premium within the quasi-experimental sub-sample.
...
errors of 0.279 and 0.182 on a sample of 291 papers.  The AFA baseline therefore
does not \textit{contradict} the NBER null, but neither does it provide
...
\medskip
\noindent\textbf{Implications for the corporate-finance regulation literature.}
Although this paper's corpus spans the full government-intervention literature
in finance, a significant share of the in-scope papers studies interventions of
direct relevance to corporate finance: mandatory disclosure requirements
(Sarbanes-Oxley, Reg FD), corporate governance regulation (say-on-pay, proxy
access, board independence mandates), short-selling restrictions, and
Dodd-Frank provisions governing capital requirements and resolution authority.
Our findings carry a specific message for practitioners and regulators who rely
on this sub-literature.  First, the directional-neutrality result means that the
published record of corporate-governance and disclosure research is unlikely to
be systematically skewed toward pro-intervention or anti-intervention conclusions
by editorial selection; both positive- and negative-finding studies on, say, the
effect of say-on-pay or mandatory auditor rotation face similar publication
probabilities.  Second, the revision-stage bunching---concentrated in
negative-finding papers and absent from the pre-publication baseline---suggests
that anti-intervention evidence in the corporate-finance regulation literature
may be subject to within-paper specification pressure before it reaches print,
even though the decision to publish is not direction-contingent.  Readers of
the corporate-governance and disclosure evidence base should therefore treat
marginal significance ($p$ just below 0.05) in anti-intervention findings with
additional scrutiny, while treating publication status as a reasonable signal
...
We document within-paper threshold bunching in published government-intervention finance research and show this bunching is absent from NBER working papers covering the same topics.  Applying the \citealt{Brodeur2016} caliper test to 7,761 $|z|$-statistics from 121 published papers (108 as permutation clusters) reveals significant full-sample threshold bunching (caliper ratio $= 1.20$, $p = 0.002$), concentrated in negative-finding papers (ratio $= 1.52$, $p < 0.001$)---papers concluding that interventions are harmful or ineffective.  Applying the identical pipeline to 8,351 test statistics from 125 NBER working papers finds no bunching (full-sample ratio $= 1.01$, $p = 0.849$; negative-finding ratio $= 1.00$, $p = 0.964$), while the overall significance rate is nearly identical across the two corpora (57.1\% vs.\ 55.5\%).  This comparison localizes the distributional difference to the published corpus, consistent with specification-search pressure at the revision stage---though compositional differences between corpora cannot be ruled out.  An important qualification is that bunching disappears when restricted to verifiable coefficient-SE rows (published ratio $= 0.81$, NBER $= 0.95$); the headline result is concentrated in z-stat-only rows (Appendix~\ref{app:coefse_caliper}).  The NBER corpus serves as a proxy for the pre-publication distribution; it is selected on NBER affiliation and should not be equated with the full population of submitted papers.

As supporting context, we examine cross-sectional direction sorting in a 291-paper comparison of two largely non-overlapping corpora (134 published, 157 NBER working papers).  We find no \textit{detectable} direction-based sorting.  Corpus-membership rates are nearly identical across positive-finding (45.6\%), negative-finding (46.1\%), and null-finding (46.7\%) papers ($\chi^2 = 0.017$, $p = 0.992$); a binary directional probit is $-0.020$ (s.e.\ 0.182, $p = 0.91$), robust across all six specifications.  These corpus-membership rates depend on sample construction and are not estimates of actual acceptance probabilities; the comparison reflects differences between two distinct populations, not the same papers tracked through the editorial filter.  The strongest predictor of corpus membership is quasi-experimental identification strategy ($0.467^{***}$, s.e.\ 0.157), consistent with editorial selection operating on methodological quality---though the LLM-coded variable captures a bundle of correlated quality attributes and cannot be attributed to identification strategy alone.
