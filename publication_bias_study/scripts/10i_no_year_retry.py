"""
10i_no_year_retry.py
---------------------
Retries abstract retrieval for all papers still missing abstracts using
CrossRef and OpenAlex with NO year filter. Fixes the year-window bottleneck
that caused multi-year-lag papers (presented at AFA years before publication)
to be missed by earlier scripts.

Content validation: at least 2 significant title words must appear in the
abstract to reject false positives.

Usage:
    python scripts/10i_no_year_retry.py          # missing papers only
    python scripts/10i_no_year_retry.py --force  # ignore cache
"""
...
CACHE_PATH = ROOT / "data" / "raw" / "afa_10i_cache.json"
LOG_PATH   = ROOT / "validation" / "abstract_10i_matches.csv"

FUZZY_MIN = 75
TIMEOUT   = 12
EMAIL     = "research@pubbiasstudy.org"
...
def openalex_search(title: str) -> str | None:
...
def crossref_search(title: str) -> str | None:
    q_norm = normalise(title)
    try:
...
def main():
    ...
    for _, row in tqdm(missing.iterrows(), total=len(missing), desc="10i no-year retry"):
        ...
        if pid in cache and not args.force:
            ...
            abstract = openalex_search(title) or ""
            if abstract:
                source = "openalex_10i"

            if not abstract:
                abstract = crossref_search(title) or ""
                if abstract:
                    source = "crossref_10i"
        ...
        if abstract:
            ...
            hits += 1
            log_rows.append({"paper_id": pid, "year": row.get("year"),
